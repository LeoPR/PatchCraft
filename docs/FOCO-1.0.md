...
**Fechado em 0.5.3**, e por um caminho melhor do que regenerar. A página virou **executável**: cada `>>>` dela roda na suíte como doctest (`tests/test_docs_usage.py`), então uma linha defasada reprova a CI em vez de enganar quem lê. As correções reais foram cinco, e o resto das falhas era formatação: a saída encostava na cerca de fechamento e o doctest lia a cerca como parte do esperado. O script gerador que faltava no git não foi recriado, porque com a página executando em lugar ele deixou de ser necessário.
