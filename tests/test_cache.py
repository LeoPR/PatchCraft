"""Tests for `patchkit.Cache` — contract from docs/THEORY.md §9.5."""
...
class TestRetry:
    ...
    def test_put_retries_on_permission_error(self, tmp_path: Path) -> None:
        ...
        with (
            patch("patchkit.cache.os.replace", side_effect=flaky_replace),
            patch("patchkit.cache.time.sleep", return_value=None),
        ):
    ...
    def test_put_gives_up_after_max_attempts(self, tmp_path: Path) -> None:
        ...
        with (
            patch("patchkit.cache.os.replace", side_effect=always_fail),
            patch("patchkit.cache.time.sleep", return_value=None),
...
class TestNonASCIIPath:
    def test_works_under_path_with_accented_chars(self, tmp_path: Path) -> None:
        ...
        weird = tmp_path / "Acadêmicos" / "patchkit-cache"
