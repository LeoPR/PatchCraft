...
2. **Nada escreve no project root.** Outputs (PNG, JSON, CSV, checkpoints, dumps) vão sempre pra `Z:\outputs\patchcraft\<slug>\` — o script cria a pasta. O project root continua limpo.
3. **Nada de `lab/` é importado por `src/patchcraft/`.** O core depende só de torch/numpy/pillow; `lab/` pode importar tudo (torchvision, matplotlib, etc) — está do lado do framework auxiliar, não do core.
