...
2. **Nada escreve no project root.** Outputs (PNG, JSON, CSV, checkpoints, dumps) vão sempre pra `Z:\outputs\patchcraft\<slug>\`, e o script cria a pasta. O project root continua limpo.
3. **Nada de `lab/` é importado por `src/patchcraft/`.** O core depende só de torch/numpy/pillow; `lab/` pode importar tudo (torchvision, matplotlib, etc), porque está do lado do framework auxiliar, não do core.
4. **Nomenclatura sugerida:** `YYYY-MM-DD-slug.{py,ipynb}`, porque o prefixo de data dá ordem cronológica no `ls`.
...
from patchcraft import extract  # reconstruct virá com M3

out = Path(r"Z:\outputs\patchcraft\2026-05-16-roundtrip-mnist")
