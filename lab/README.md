...
2. **Nada escreve no project root.** Outputs (PNG, JSON, CSV, checkpoints, dumps) vão sempre pra `Z:\outputs\patchkit\<slug>\` — o script cria a pasta. O project root continua limpo.
3. **Nada de `lab/` é importado por `src/patchkit/`.** O core depende só de torch/numpy/pillow; `lab/` pode importar tudo (torchvision, matplotlib, etc) — está do lado do framework auxiliar, não do core.
...
from patchkit import extract  # reconstruct virá com M3

out = Path(r"Z:\outputs\patchkit\2026-05-16-roundtrip-mnist")
